...
allMEPtweetsDF$text <- stringr::str_replace_all(
  string = allMEPtweetsDF$text,
  pattern = stringr::regex(pattern = "@[[:alnum:]]+|#[[:alnum:]]+|http[[:graph:]]+|RT ", ignore_case = FALSE), replacement = "")

allMEPtweetsDF$text <- stringr::str_replace_all(string = allMEPtweetsDF$text, pattern = stringr::fixed("&amp;"), replacement = "&") 
...
```{r}
dataFolder <- "/srv/shiny-server/QuoteFinder/data"
...
lang <- as.list(lang$lang)

saveRDS(object = lang, file = file.path(dataFolder, "lang.rds"))   

hashtags <- unlist(lang)

hashtags <- vector("list", length = length(lang))
hashtags <- setNames(object = hashtags, nm = unlist(lang))
for (i in seq_along(lang)) {
  ...
  hashtags[[i]] <- as.list(temp$hashtags)
}
