...
graph <- dataset %>% wordcloud2(color = colour, size = 2)

filename <- file.path("img", paste(Sys.time(), "wordcloud.png", sep = "-"))

saveWidget(graph,"tmp.html",selfcontained = FALSE)
webshot("tmp.html", filename, delay = 10, vwidth = 2048, vheight = 1536)
...
```{r}
# Post tweet
post_tweet(status = paste0("What have MEPs been tweeting about in the last week? Here's what they said in English. Explore the tweets also in other languages at https://www.europeandatajournalism.eu/Tools-for-journalists/Quote-Finder/"), media = filename, token = twitter_token)
```
