...
bluesFunc <- colorRampPalette(colors = c("#ff0000", "#ffff00"))
blues <- bluesFunc(5)
...
randomTopHashtag <- unlist(sample(head(trendingHashtags$es, 2), 1))
...
dataset <- dataset %>%  
  ...
  when(is.element(el = "es", set = stopwords::stopwords_getlanguages(source = "stopwords-iso")) ~
         anti_join(., data_frame(word = stopwords::stopwords(language = "es", source = "stopwords-iso")), by = "word"),
       ~ .) %>% 
...
filename <- file.path("img", paste(Sys.time(), "wordcloud2 trending hashtag", hashtag, "ES.png", sep = "-"))
...
```{r}
# Post tweet
post_tweet(status = paste0("Here's one of the trending hashtags among MEPs tweeting in Spanish this week: #", hashtag, ". Find out more at https://www.europeandatajournalism.eu/Tools-for-journalists/Quote-Finder/"), media = filename, token = twitter_token)
```